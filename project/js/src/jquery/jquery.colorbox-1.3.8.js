...
//			$close = $div("Close"),
		...
		$loadingBay = $div('-loading-bay', 'position:absolute; width:9999px; visibility:hidden; display:none');
		...
		interfaceWidth = $leftBorder.width() + $rightBorder.width() + $content.outerWidth(true) - $content.width() + 6;
		loadedHeight = $loaded.outerHeight(true);
		loadedWidth = $loaded.outerWidth(true) - 6;
	...
	publicMethod.position = function (speed, loadedCallback) {
		...
		function modalDimensions(that) {
			...
			if (that.style.width == '') {
				...
				var w = that.style.width;
				w = (parseInt(w.replace('px', '')) + 6).toString() + 'px';
				$content[0].style.width = w;
			}
	...
	publicMethod.load = function () {
		...
		if (settings.inline) {
			...
			$div().appendTo($loadingBay).load(href, settings.post_data, function (data, status, xhr) {
