...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Workarround for http://south.aeracode.org/ticket/478
        if db.backend_name != "sqlite3":
            # Adding index on 'Inventory', fields ['is_new']
            db.create_index('inventory_inventory', ['is_new'])

            # Adding index on 'Inventory', fields ['buy_only']
            db.create_index('inventory_inventory', ['buy_only'])

    def backwards(self, orm):
        if db.backend_name != "sqlite3":
            # Removing index on 'Inventory', fields ['is_new']
            db.delete_index('inventory_inventory', ['is_new'])

            # Removing index on 'Inventory', fields ['buy_only']
            db.delete_index('inventory_inventory', ['buy_only'])
