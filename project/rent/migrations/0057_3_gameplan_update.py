...
class Migration(DataMigration):
    def forwards(self, orm):
        """
        Updating 3 Game Plan price to 27.95, removing old 3 GP,
        transfering accounts from old 3 GP to new one.
        """
        from project.new_rent.models import MemberRentalPlan, RentalPlan, RentalPlanStatus
        old_unlimited3_qs = RentalPlan.objects.filter(pk=2)
        if old_unlimited3_qs:
            old_unlimited3 = RentalPlan.objects.filter(pk=2)[0]
        unlimited3_qs = RentalPlan.objects.filter(pk=19)
        unlimited3 = unlimited3_qs[0]

        unlimited3_qs.update(first_month="27.95", thereafter_months="27.95", slug="unlimited3")
        if old_unlimited3_qs:
            MemberRentalPlan.objects.filter(plan=old_unlimited3.pk, status=RentalPlanStatus.Active).update(
                plan=unlimited3.pk,
                next_payment_amount=unlimited3.thereafter_months
            )
            old_unlimited3_qs.delete()
