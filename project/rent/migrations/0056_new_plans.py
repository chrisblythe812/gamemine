...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        BaseRentalPlan.objects.filter(pk=0).update(slug="old_limited")
        BaseRentalPlan.objects.filter(pk=0).update(out_per_month=2)
        BaseRentalPlan.objects.filter(pk=1).update(slug="old_unlimited")
        BaseRentalPlan.objects.filter(pk=1).update(slug="old_unlimited")
        BaseRentalPlan.objects.filter(pk=2).update(slug="unlimited3")
        BaseRentalPlan.objects.filter(pk=3).update(slug="unlimited_prepaid3")
        BaseRentalPlan.objects.filter(pk=4).update(slug="unlimited_prepaid6")

    def backwards(self, orm):
        BaseRentalPlan.objects.filter(pk__in=[5, 6, 7]).delete()
        BaseRentalPlan.objects.filter(pk=0).update(slug=None)
        BaseRentalPlan.objects.filter(pk=1).update(slug=None)
        BaseRentalPlan.objects.filter(pk=1).update(slug=None)
        BaseRentalPlan.objects.filter(pk=3).update(slug=None)
        BaseRentalPlan.objects.filter(pk=4).update(slug=None)
